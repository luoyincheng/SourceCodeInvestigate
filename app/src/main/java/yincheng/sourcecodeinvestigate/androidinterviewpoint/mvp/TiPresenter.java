...
import yincheng.sourcecodeinvestigate.androidinterviewpoint.mvp.internal.OneTimeRemovable;
import yincheng.sourcecodeinvestigate.androidinterviewpoint.mvp.internal.UiThreadExecutor;
import yincheng.sourcecodeinvestigate.androidinterviewpoint.mvp.test.TiTestPresenter;
...
public abstract class TiPresenter<V extends TiView> {
    ...
    */
   @VisibleForTesting final List<TiLifecycleObserver> mLifecycleObservers = new ArrayList<>();
    ...
    * <p>
    * This Executor is most likely the {@link UiThreadExecutor}
    ...
    * Unfortunately a ui executor thread isn't available in unit test. Instead a mock
    * implementation
         ...
         switch (oldState) {
            ...
            case VIEW_DETACHED:
               if (newState == State.VIEW_ATTACHED) {
                  ...
                  throw new IllegalStateException("Can't move to state " + newState
                        + ", the allowed states after VIEW_DETACHED are VIEW_ATTACHED or " +
                        "DESTROYED");
               }
            case VIEW_ATTACHED:
