...
import yincheng.sourcecodeinvestigate.androidinterviewpoint.mvp.util.AbstractInvocationHandler;
...
/**
 * Created by yincheng on 2018/5/23/12:12.
 * github:luoyincheng
 */
public class LoggingIntercepter implements BindViewInterceptor {
    ...
    * logging from {@link TiLog} or set your own logger with {@link LoggingIntercepter#LoggingIntercepter(TiLog.Logger)}
    */
    ...
    * {@link TiLog.Logger} interface
    *
    * @param logger custom logger, {@link TiLog#LOGCAT} or {@link TiLog#NOOP} to disable logging.
    */
   public LoggingIntercepter(@Nullable final TiLog.Logger logger) {
      if (logger == null) {
         mLogger = TiLog.NOOP;
      } else {
         mLogger = logger;
