package yincheng.sourcecodeinvestigate.jobintentservice;

/**
 * Created by yincheng on 2018/5/9/16:18.
 * github:luoyincheng
 */
...
public abstract class JobIntentService extends Service {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            mJobImpl = new JobIntentService.JobServiceEngineImpl(this);
    ...
    public int onStartCommand(@Nullable Intent intent, int flags, int startId) {
        if (mCompatQueue != null) {
            ...
            synchronized (mCompatQueue) {
                mCompatQueue.add(new JobIntentService.CompatWorkItem(intent != null ? intent : new Intent(),
                        startId));
    ...
    static JobIntentService.WorkEnqueuer getWorkEnqueuer(Context context, ComponentName cn, boolean hasJobId,
                                                                                int jobId) {
        JobIntentService.WorkEnqueuer we = sClassWorkEnqueuer.get(cn);
        if (we == null) {
            if (Build.VERSION.SDK_INT >= 26) {
                if (!hasJobId) {
    ...
    void ensureProcessorRunningLocked(boolean reportStarted) {
        if (mCurProcessor == null) {
            mCurProcessor = new JobIntentService.CommandProcessor();
